...
Vector4d sample_noise_from_plane(const Vector4d& cov ){
...
struct PlaneItem: public WorldItem{
  PlaneItem(OptimizableGraph* graph_, int id) : WorldItem(graph_){
    VertexPlaneNM* p=new VertexPlaneNM();
    p->setId(id);
    graph()->addVertex(p);
    setVertex(p);
  } 
...
struct PlaneSensor: public Sensor{
  ...
  virtual bool sense(WorldItem* wi, const Isometry3d& position){
    ...
    VertexPlaneNM* planeVertex=dynamic_cast<VertexPlaneNM*>(pi->vertex());
    Plane3D worldPlane=planeVertex->estimate();
    
    Plane3D measuredPlane=sensorPose.inverse()*worldPlane;

    EdgeSE3PlaneNMSensorCalib* e=new EdgeSE3PlaneNMSensorCalib();
    e->vertices()[0]=robotVertex;
    e->vertices()[1]=planeVertex;
    e->vertices()[2]=_offsetVertex;
    Vector4d noise = sample_noise_from_plane(_nplane);
    e->setMeasurement(measuredPlane.coeffs() + noise);
    ...
    EdgePlaneNormalize* eNorm = new EdgePlaneNormalize;
    eNorm->setVertex(0,planeVertex);
    Eigen::Matrix<double,1,1> info1;
    info1 << 1e3;
    eNorm->setInformation(info1);
    robot()->graph()->addEdge(eNorm);
    return true;
  }
...
int main (int argc  , char ** argv){
  ...
  static_cast<VertexPlaneNM*>(pi->vertex())->setEstimate(plane);
