...
namespace Slam3dAddons {
  ...
  bool VertexPlaneNM::read(std::istream& is) {
    Vector4d lv;
    for (int i=0; i<4; i++)
      is >> lv[i];
  ...
  bool VertexPlaneNM::write(std::ostream& os) const {
  ...
  VertexPlaneNMDrawAction::VertexPlaneNMDrawAction(): DrawAction(typeid(VertexPlaneNM).name()){
  }

  bool VertexPlaneNMDrawAction::refreshPropertyPtrs(HyperGraphElementAction::Parameters* params_){
  ...
  HyperGraphElementAction* VertexPlaneNMDrawAction::operator()(HyperGraph::HyperGraphElement* element, 
    ...
    VertexPlaneNM* that = static_cast<VertexPlaneNM*>(element);
