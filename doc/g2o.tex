...
\begin{description}
\item \verb+BaseVertex+ is a CRTP template class
  \verb+BaseVertex<Derived, D, T>+ that implements most of the common
  vertex functionality for the system. Here, \verb+Derived+ is the
  concrete vertex type, \verb+D+ is the minimal update dimension of
  $\bDeltax_i$ (with \verb+-1+ indicating a dynamic dimension assigned
  at runtime), and \verb+T+ is the estimate type stored by the vertex.
  It provides mapped Hessian memory and the right-hand side vector of
  the linearized problem, as well as a backup stack for save/restore
  of previous estimates. The derived class must implement
  \verb+oplusImpl(const VectorX::MapType& v)+ to apply the perturbation
  $\bDeltax_i$ to the internal estimate. The base class also updates
  cached data automatically after \verb+oplus+ or \verb+setEstimate+
  operations.

  The class relies on the generic \verb+TypeTraits<T>+ trait to convert
  the estimate type to and from dense vectors. This enables the
  framework to handle estimate serialization and minimal-vector
  conversion for both vertices and edges without manual type-specific
  boilerplate.

\item \verb+BaseUnaryEdge<D, E, VertexXi>+ is a template class to
  model a unary hyper-edge, which can be used to represent a prior.
  It is a thin wrapper around \verb+BaseFixedSizedEdge<D, E, VertexXi>+
  and provides a default implementation of \verb+linearizeOplus()+
  that computes numeric Jacobians. The derived class should implement
  \verb+computeError()+ to store the current residual in the edge's
  \verb+error_+ vector.

\item \verb+BaseBinaryEdge<D, E, VertexXi, VertexXj>+ is a template class
  that models a binary constraint, namely an error function in the form
  $\be_k(\bx_{k_1}, \bx_{k_2})$. It is a thin wrapper around
  \verb+BaseFixedSizedEdge<D, E, VertexXi, VertexXj>+, and it similarly
  implements numeric Jacobians via a default \verb+linearizeOplus()+
  method if the derived class does not override it. The derived class
  should implement \verb+computeError()+.

\item \verb+BaseMultiEdge<D, E>+ is a template alias for
  \verb+BaseVariableSizedEdge<D, E>+, and it models a constraint that
  connects an arbitrary number of vertices. It requires the measurement
  type and dimension as template parameters, and the concrete edge
  implementation must resize the connected vertices appropriately.
  This class also provides a default numeric \verb+linearizeOplus()+
  implementation and expects \verb+computeError()+ to populate the
  edge's \verb+error_+ vector.
\end{description}

\subsubsection{TypeTraits and automatic data conversion}
The core optimizer classes rely on the generic \verb+TypeTraits<T>+
trait to convert estimate and measurement types to and from dense
vectors. This trait supplies the full vector dimension,
minimal vector dimension, and the conversion functions that are used by
\verb+BaseVertex+, \verb+BaseEdge+, and the related input/output
interfaces.

For built-in scalar and Eigen vector types, the library already provides
specializations. For a custom type \verb+YourType+, a
\verb+TypeTraits<YourType>+ specialization must define:
\begin{itemize}
\item \verb+kVectorDimension+ and \verb+kMinimalVectorDimension+
\item \verb+VectorType+ and \verb+MinimalVectorType+
\item \verb+toVector+ and \verb+toMinimalVector+
\item \verb+fromVector+ and \verb+fromMinimalVector+
\item \verb+toData+ and \verb+toMinimalData+
\end{itemize}
This makes the framework manage estimate serialization, minimal
parameterization, and edge measurement conversion automatically.
...
the error should be cached in the edge's \verb+error_+ vector.

If \verb+robustKernel()+ is set to true for a particular active edge,
\verb+robustifyError()+ is called and the error vector is robustified
...
After defining the group type, we also provide a \verb+TypeTraits<SE2>+
specialization. This tells the framework how to convert the custom
\verb+SE2+ estimate type to and from a dense 3-vector, and how to
serialize it for the optimizer's internal data structures.

\begin{lstlisting}[float,label=lst:se2typetraits,caption=TypeTraits for the
  tutorial SE2 estimate type]
namespace g2o {
...
vertices. To this end we extend the CRTP class
\verb+BaseVertex<Derived, D, T>+, and we derive the classes
\verb+VertexSE2+ to represent a robot pose and \verb+VertexPointXY+
to represent a point landmark in the plane. The class definition for
robot pose vertices is reported in Listing~\ref{lst:vertexse2}.
The pose vertex extends the template specialization
\verb+BaseVertex<VertexSE2, 3, SE2>+. This tells \gopt{} that the
minimal update dimension is 3 and that the stored estimate type is
\verb+SE2+. The derived class must implement
\verb+oplusImpl(const VectorX::MapType& update)+ to apply the
perturbation vector. In this tutorial the vertex estimate is
initialized in the constructor and the perturbation is interpreted as
a small increment $(\Delta x \; \Delta y \; \Delta \theta)^T$ in
$SE(2)$, which is converted to an \verb+SE2+ increment and composed
with the current estimate.

\begin{lstlisting}[float,label=lst:vertexse2,caption=Vertex representing
  a 2D robot pose]
class VertexSE2 : public BaseVertex<VertexSE2, 3, SE2>
{
  public:
    VertexSE2() { estimate_ = SE2(); }
...
\begin{lstlisting}[float,label=lst:vertexxy,caption=Vertex representing
  a 2D landmark]
class VertexPointXY : public BaseVertex<VertexPointXY, 2, Eigen::Vector2d>
{
  public:
    VertexPointXY() { estimate_.setZero(); }
...
both edges are binary edges, we extend the class \verb+BaseBinaryEdge<>+.
To represent an odometry edge (see Listing~\ref{lst:edgese2}) that
connects two \verb+VertexSE2+, we need to extend \verb+BaseBinaryEdge<>+
using the types of the connected vertices in the template parameter list.
The order of the vertex types is significant: the first vertex is index
0 and the second vertex is index 1. Inside \verb+computeError()+, these
vertices can be accessed explicitly via \verb+vertexXnRaw<0>()+ and
\verb+vertexXnRaw<1>()+.

The measurement itself is represented by an \verb+SE2+ of dimension 3
and stored in \verb+measurement_+. The edge should override
\verb+computeError()+ to fill \verb+error_+ with a 3D residual vector.
...
by \gopt{}. However, if you want to speed up execution after everything
works, you can override \verb+linearizeOplus()+ and provide analytic
Jacobians.
...
\verb+VertexPointXY+, using an \verb+Eigen::Vector2d+ measurement of
dimension 2. In this example, the edge also uses a cached transform to
convert the landmark into the robot frame.
