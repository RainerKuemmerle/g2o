...
\newcommand{\gopt}{\ensuremath{\mathbf{g^2o}}}
...
\item \verb+BaseVertex+ templatizes the dimension of a parameter block
  ...
  save/restore parts of the graph.  The method \verb+oplus(double* v)+
...
void __attribute__ ((constructor)) init_types_slam2d(void)
{
...
Using the GCC compiler we can define a constructor function inside our
libraries which will take care of this procedure. See
Listing~\ref{lst:inittypes} for an example, the full example can be
found in \verb+types_slam2d.cpp+.  The first parameter given to the
function \verb+registerType+ specifies the tag under which a certain
vertex / edge is known. \gopt\ will use this information while loading
files and for saving the current graph into a file. In the example given
in Listing~\ref{lst:inittypes} we register the tags \verb+VERTEX_SE2+
and \verb+EDGE_SE2+ with the classes \verb+VertexSE2+ and
\verb+EdgeSE2+, respectively.
...
class PCGSolverCreator : public AbstractOptimizationAlgorithmCreator
{
  public:
    ...
    {
      // create the solver, see g2o/solver_pcg/solver_pcg.cpp for the full code
    }
};
...
void __attribute__ ((constructor)) init_solver_csparse()
{
  ...
  factory->registerSolver(new PCGSolverCreator(OptimizationAlgorithmProperty("pcg",
     "PCG solver using block-Jacobi pre-conditioner (variable blocksize)",
     "PCG", false, -1, -1)));
  //...
}
\end{lstlisting}
...
to register types and solvers, respectively. We assume that by loading
the libraries register the types and the solvers via their respective
constructors to the system. Listing~\ref{lst:inittypes} shows how to
register types to the system and Listing~\ref{lst:initsolvers} is an
example, which shows how to register a solver via the plug-in
architecture.

For loading dynamic library containing types or solvers, we support two
different methods:
\begin{itemize}
...
\psfrag{u12}{$\bz^\mathrm{l}_{1,2}$}
\psfrag{utt1}{$\bz^\mathrm{l}_{t-1,t}$}
...
\verb+setToOrigin()+ that resets the estimate to a known
configuration, and the method \verb+\oplus(double*)+.  The method
...
\end{lstlisting}
The final thing we should do to make our system operational, is to
