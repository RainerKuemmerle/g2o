...
void declareG2OBatchStatistics(py::module& m) {
  py::class_<G2OBatchStatistics>(m, "G2OBatchStatistics")
      ...
      // nonlinear part
      .def_readwrite("time_residuals", &G2OBatchStatistics::timeResiduals)              // double
      .def_readwrite("time_linearize", &G2OBatchStatistics::timeLinearize)              // double
      .def_readwrite("time_quadratic_form", &G2OBatchStatistics::timeQuadraticForm)     // double
      .def_readwrite("levenberg_iterations", &G2OBatchStatistics::levenbergIterations)  // int

      // block_solver (constructs Ax=b, plus maybe schur)
      .def_readwrite("time_schur_complement", &G2OBatchStatistics::timeSchurComplement)  // double
      ...
      .def_readwrite("time_linear_solution", &G2OBatchStatistics::timeLinearSolution)  // double
      .def_readwrite("time_linear_solver", &G2OBatchStatistics::timeLinearSolver)      // double
      ...
      .def_readwrite("time_iteration", &G2OBatchStatistics::timeIteration)  // double
      .def_readwrite("time_marginals", &G2OBatchStatistics::timeMarginals)  // double

      // information about the Hessian matrix
      .def_readwrite("hessian_dimension", &G2OBatchStatistics::hessianDimension)           // size_t
      .def_readwrite("hessian_pose_dimension", &G2OBatchStatistics::hessianPoseDimension)  // size_t
