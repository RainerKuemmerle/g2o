...
void delcareEstimatePropagator(py::module& m) {
  py::class_<EstimatePropagatorCost>(m, "EstimatePropagatorCost")
      ...
      .def("__call__", &EstimatePropagatorCost::operator(), "edge"_a, "from"_a, "to"_a,
           py::keep_alive<1, 2>(), py::keep_alive<1, 3>(),
           py::keep_alive<1, 4>())  // (OptimizableGraph::Edge* edge, const
                                    // OptimizableGraph::VertexSet& from, OptimizableGraph::Vertex*
                                    // to_) -> double
  ...
  py::class_<EstimatePropagator::PropagateAction>(cls, "EstimatePropagateAction")
      .def(py::init<>())
      .def("__call__", &EstimatePropagator::PropagateAction::operator(), py::keep_alive<1, 2>(),
           py::keep_alive<1, 3>(),
           py::keep_alive<1, 4>())  // (OptimizableGraph::Edge* edge, const
                                    // OptimizableGraph::VertexSet& from, OptimizableGraph::Vertex*
                                    // to_) -> double
  ...
  py::class_<EstimatePropagator::AdjacencyMapEntry>(cls, "EstimatePropagatorAdjacencyMapEntry")
      .def(py::init<>())
      ...
      .def("frontier_level", &EstimatePropagator::AdjacencyMapEntry::frontierLevel);
  ...
  cls.def("visited", &EstimatePropagator::visited);             // -> OptimizableGraph::VertexSet&
  ...
  cls.def("propagate",
          (void (EstimatePropagator::*)(
              OptimizableGraph::VertexSet&, const EstimatePropagator::PropagateCost&,
              const EstimatePropagator::PropagateAction&, double, double)) &
              EstimatePropagator::propagate,
          "vset"_a, "cost"_a, "action"_a, "maxDistance"_a = std::numeric_limits<double>::max(),
          "maxEdgeCost"_a = std::numeric_limits<double>::max(), py::keep_alive<1, 2>(),
          py::keep_alive<1, 3>(), py::keep_alive<1, 4>());
}
